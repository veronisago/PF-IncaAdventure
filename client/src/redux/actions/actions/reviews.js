...
export function getReviews(){
  return async function (dispatch){
    try {
      let reviews = await axios.get(`${REACT_ENV_BASE_URL}/reviews`, {});
      return dispatch({
        type: GET_REVIEWS,
...
export function getReviewsByRating(name){
  ...
  return async function (dispatch){
    try {
      let reviewsByName = await axios.get(`${REACT_ENV_BASE_URL}/reviews?name=${name}`, {});
      return dispatch({
...
export function getReviewsByWord(word){
  ...
  return async function (dispatch){
    try {
      let reviewsByWord = await axios.get(`${REACT_ENV_BASE_URL}/reviews?word=${word}`, {});
      return dispatch({
...
export function getReviewsById(id){
  return async function (dispatch){
    try {
      let reviewsById = await axios.get(`${REACT_ENV_BASE_URL}/reviews/${id}`, {});
      return dispatch({
